...
pos_f_expect_raw=$(find $data_path -name "pos_[0-9][0-9]_fexpect.txt" | sort | tr '\n' ' ')
...
IFS=' ' read -r -a pos_f_expect <<< "$pos_f_expect_raw"
...
if [ "$pos_in_c" -ne "$pos_f_expect_c" ]; then
    echo "pos_in and pos_f_expect files count is not equal! Aborting tests..."
    exit $ERR
fi
...
for i in "${!pos_in[@]}"; do
    echo $"Test ${pos_in[$i]}"
    eval ./pos_case.sh "${pos_in[$i]}" "${pos_out[$i]}" "${pos_args[$i]}" "${pos_f_expect[$i]}" "${pos_f_expect[$i]/expect/out}"
    res=$?
    if [ $res -eq 0 ]; then
    ...
    if [ -n "$USE_VALGRIND" ]; then
        memory=$(grep -Eo "ERROR SUMMARY: [0-9]+" < ../../out/v.temp | cut -d ' ' -f 3)
        if [ "$memory" -eq 0 ]; then
