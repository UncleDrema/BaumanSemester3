...
void output_add_result(long long unsigned sparse_time, long long unsigned matrix_time, sparse_matrix_t *sparse)
{
    printf("Repeat 1000x:\n");
    printf("---------------------------------------------\n");
    ...
    printf("|--------------+------------------+---------|\n");
    printf("|    sparse    |");
    output_llu_num_center(sparse_time, 18);
    printf("|");
    output_num_center((sparse->el_count * 2 * sizeof(size_t) + ((sparse->cols_count + 1) * sizeof(int))), 9);
    printf("|\n");
    printf("|--------------+------------------+---------|\n");
    printf("|    Default   |");
    output_llu_num_center(matrix_time, 18);
    printf("|");
    output_num_center((sparse->cols_count * sparse->rows_count) * sizeof(int), 9);
    printf("|\n");
    printf("---------------------------------------------\n");
}
...
static int check_line(char *line, bool digit)
{
    for (char *p = line; *p; ++p)
    {
...
static int read_line(FILE *f, char *line, bool digit)
{
...
{
    char tmp[DIGITS + 2];
    int error_code = read_line(stdin, tmp, true), counter = 0;
    if (error_code)
    {
        return error_code;
    }
